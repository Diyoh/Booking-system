            ...
            <div class="p-6 bg-white border-b border-gray-200">
                <form method="POST" action="{{ route('admin.events.update', $event->id) }}" enctype="multipart/form-data">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <select name="location" id="location" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                                ...
                                @foreach($locations as $loc)
                                    <option value="{{ $loc->name }}" {{ old('location', $event->location) == $loc->name ? 'selected' : '' }}>
                                        {{ $loc->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            <label for="ticket_price" class="block text-sm font-medium text-gray-700">Ticket Price (FCFA)</label>
                        ...
                        <div class="md:col-span-2 bg-gray-50 p-4 rounded border border-gray-200 mt-4">
                            ...
                            @if($event->image_url)
                                <div class="mb-4">
                                    ...
                                    <img src="{{ Str::startsWith($event->image_url, ['http://', 'https://']) ? $event->image_url : asset('storage/' . $event->image_url) }}" alt="Front Image" class="h-32 w-auto object-cover rounded shadow">
                                </div>
                            @endif
